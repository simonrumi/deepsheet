...
export const getCellFromCells = ({ row, column, cells }) => arrayContainsSomething(cells) 
...
export const makeArr = length => new Array(length);
...
export const getSymbolValue = ({ description, obj }) => {
	return R.reduce(
		(accumulator, sym) => {
			if (sym?.description === description) {
				return R.reduced(obj[sym]);
			}
			return accumulator;
		},
		null, //default value
