      ...
      newCells = forLoopReduce(
         (accumulator, rowIndex) =>
            R.pipe(
               ...
               getCellFromState(rowIndex, movedColumnIndex),
               R.assoc('column', columnIndex, {})
...
export default (state) => {
   ...
   const insertAfterIndex = state.sheet.columnMovedTo;
   ...
   const makeNewSheetItem = (createSheetItemName) =>
      R.pipe(
         createSheetItemName,
         makeNewSheetItemFromMap(columnUpdateArr, state.sheet)
      )(COLUMN_AXIS);
