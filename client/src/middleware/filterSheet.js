/*
TODO: MAYBE refactor store so that columnVisibility, rowVisibility, columnFilters and rowFilters
...
const isCellShownByFilter = (cell, filter) => {
...
const getVisibilityForCellsInAxisItem = (data, axis, itemIndex) => {
	...
	// so we can return an object saying just one column/row is visible, and this will be reduced in getAxisVisibilityValue
	// to a single true value for the whole row or column
	if (R.isNil(filters)) {
		return { 0: true }; //
	}

	// note R.mapObjIndexed sends its 1st argument, a function, the parameters (value, key, obj), but we're not using the obj
	return R.mapObjIndexed((filter, filterIndex) => {
		const cellKey = createCellKey(data, axis, itemIndex, filterIndex);
		const cell = R.prop(cellKey, getStateFromData(data));
		return isCellShownByFilter(cell, filter);
	}, getFilters(getOtherAxis(axis), getSheetFromData(data)));
};
