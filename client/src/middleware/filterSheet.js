...
const createCellKey = (data, oppositeAxisItemIndex, filterIndex) => {
   const rowIndex =
      getOppositeAxis(data) === ROW_AXIS ? oppositeAxisItemIndex : filterIndex;
   const colIndex =
      getOppositeAxis(data) === COLUMN_AXIS
         ? oppositeAxisItemIndex
         : filterIndex;
...
// for each filter in axisFilters // TODO this is working up to and including this function, continue testing below
const setVisibilityForOppositeAxisItem = (data, itemIndex) =>
   data.store.dispatch(
      createOppositeAxisVisibilityUpdateAction(data, itemIndex)
   );

// for each oppositeAxis Item
const filterAllItemsInOppositeAxis = (data, axis) => {
   R.times(
      setVisibilityForOppositeAxisItem(data),
      getTotalForAxis(axis, data.storeState.sheet)
   );
};

// if axisFilters are not empty
const applyAllFiltersInAxis = R.curry((data, axis) => {
   const axisFilters = this.getFilters(axis, data.storeState.sheet);
   return R.isEmpty(axisFilters)
      ? nothing
      : this.filterAllItemsInOppositeAxis(data, axis);
});

// for each axis
const filterWholeSheet = data => {
   const axis = this.getAxis(data);
   const oppositeAxis = this.getOppositeAxis(data);
   R.forEach(this.applyAllFiltersInAxis(data), [axis, oppositeAxis]);

   //return checkEachItemAgainstFilter(getAxis(data), store.getState().sheet);
};
...
const updateAllCellsInOppositeAxis = (newVisibility, index, data) => {
   ...
   R.map(cellKey => {
      ...
      const dispatchActions = colIndex => {
         const newCellVisibility =
            defaultToTrue(thisAxisVisibility[axesIndicies[thisAxis]]) &&
            newVisibility;
...
export default store => next => action => {
   ...
   switch (action.type) {
      case UPDATED_FILTER:
         //filterWholeSheet(action.payload, store);
