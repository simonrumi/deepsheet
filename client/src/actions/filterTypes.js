export const HIDE_FILTERED = 'hide_filtered';
export const CLEAR_ALL_FILTERS = 'clear_all_filters';
export const CHANGED_FILTER_VALUE = 'changed_filter_value';
export const CHANGED_HIDE_BLANKS_VALUE = 'changed_hide_blanks_value';
export const CHANGED_REGEX_VALUE = 'changed_regex_value';
export const CHANGED_CASE_SENSITIVE_VALUE = 'changed_case_sensitive_value';
export const FILTER_EDIT_CANCELLED = 'filter_edit_cancelled';
export const TOGGLED_SHOW_FILTER_MODAL = 'toggled_show_filter_modal';