...
const moveRowContent = (rowIndexToMove, insertBelowIndex, sheet) => {
	const rowUpdateArr = mapOldIndicesToNewIndicies(rowIndexToMove, insertBelowIndex, sheet.totalRows);
	return rowUpdateArr; /// for a first test just console log out the array of new indices
};

const moveRow = (rowIndexToMove, insertBelowIndex, sheet) => {
	if (rowIndexToMove === insertBelowIndex) {
		return null;
	}
	return moveRowContent(rowIndexToMove, insertBelowIndex, sheet); /// for a first test just console log out the array of new indices
