 ...
 ****/
const createFilterObjKey = filterData => filterData.rowIndex || filterData.colIndex;

const createFilterObj = filterData => R.omit(['showFilterModal', 'colIndex', 'rowIndex'], filterData);

const checkFilterData = R.converge(R.and, [R.has('rowIndex'), R.has('colIndex')]);

const checkedFilterObj = R.ifElse(checkFilterData, createFilterObj, () => null);

export const getFilterName = filterData => (R.prop('rowIndex', filterData) ? 'rowFilters' : 'columnFilters');

const updateFilterValue = (filterData, sheet) =>
	R.assoc(createFilterObjKey(filterData), checkedFilterObj(filterData), sheet[getFilterName(filterData)]);

export const replaceFilterEntry = (filterData, sheet) =>
	R.assoc(getFilterName(filterData), updateFilterValue(filterData, sheet), sheet);
...
// 	R.converge(R.subtract, [getTotalForAxis, getNumHiddenItemsForAxis])(axis, sheet);
...
export const getRequiredNumItemsForAxis = (axis, sheet) => {
	...
	if (typeof visibleItems === 'number') {
		return visibleItems + 1; // adding 1 for the row/column to hold the "+" icon
	}
	return visibleItems;
};
