...
import { forLoopMap, mapWithIndex, reduceWithIndex } from '../helpers';

export const makeNewSheetObjectFromMap = R.curry((axisUpdateMapping, sheet, objectName) =>
	R.reduce(
		(accumulator, axisMap) => {
...
export const buildObject = R.pipe(
	createArray,
	R.mergeAll
);

export const reorderIndicies = (axisIndexToMove, insertAfterIndex, totalColumns) => {
	...
	if (axisIndexToMove < insertAfterIndex) {
		...
		const newSecondPart = R.slice(axisIndexToMove + 1, insertAfterIndex + 1, initialArray); // from axisIndexToMove to insertIndex will be moved 1 closer to start
	...
	if (axisIndexToMove > insertAfterIndex) {
		...
		const newThirdPart = R.slice(insertAfterIndex + 1, axisIndexToMove, initialArray); // from insertAfterIndex + 1 to axisIndexToMove - 1 will be moved 1 closer to end
		const newEndPart = R.slice(axisIndexToMove + 1, totalColumns, initialArray); // greater than axisIndexToMove is untouched
		return R.pipe(
			R.concat,
